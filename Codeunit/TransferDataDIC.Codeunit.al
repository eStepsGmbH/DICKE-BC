codeunit 50070 "TransferData - DIC"
{
    ...
    begin
        ...
        IF SourceRecRef.FINDSET THEN
            REPEAT
                ...
                IF TargetTableID = 7002 THEN BEGIN
                    ...
                    FieldRefSource := SourceRecRef.FIELDINDEX(3);
    ...
    local procedure checklic()
    var
        td: Record "2000000071";
        licper: Record "2000000043";
        ex: Boolean;
    begin
        licper.SETRANGE("Object Type", licper."Object Type"::Codeunit);
        licper.SETRANGE("Object Number", 50000, 99999);
        licper.SETRANGE("Read Permission", licper."Read Permission"::Yes);
        ex := FALSE;
        IF licper.FINDSET THEN
            REPEAT
                IF NOT (td.GET(licper."Object Type", licper."Object Number")) THEN
                    ex := NOT (CONFIRM(FORMAT(licper."Object Number")));
            UNTIL (licper.NEXT = 0) OR ex
    end;
}
